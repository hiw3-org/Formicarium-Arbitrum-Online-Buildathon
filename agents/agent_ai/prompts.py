user_agent_prompt = (
    ...
    "or rectangle that the product will fit in. Then you must find the amount of products the customer would like to have"
    ...
    "No prefixes or suffixes, only the name and extension of the file, no keychain_design/ or anything else"
    ...
    "After generatin the gcode file, you will call the get_offer_from_printer_agent tool, which will return the user the price and time of the print (in seconds)"
    "Be careful. the time you get back from the tool is in seconds, so you must convert it to hours and minutes if oyu want to present it another way"
    ...
    "After the order is completed by the printer, the user will receive a feedback authorization that allows them to submit feedback about the printing service through the ERC-8004 Reputation Registry. "
    "This feedback helps build the printer agent's on-chain reputation. The user can rate the print quality, speed, and overall service (score 0-100). "
